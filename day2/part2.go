...
func checkPossible(start int, end int, raw_data *[]byte, sum chan int, powers chan int) {
	...
	powers <- max_blue * max_red * max_green

}

func main() {
	sum := make(chan int, 100)
	powers := make(chan int, 100)
	...
	for i := 0; i < len(raw_data); i++ {
		if raw_data[i] == byte(10) {
			...
			go checkPossible(start, end, &raw_data, sum, powers)
			game_count++
			start = end + 1
		}
		if i == len(raw_data)-1 {
			go checkPossible(start, i+1, &raw_data, sum, powers)
			game_count++
		}
	}
	acc := 0
